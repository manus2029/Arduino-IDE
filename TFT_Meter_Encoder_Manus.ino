...
// #########################################################################
// Pin definitions
const uint8_t ENC_PUSH = 32;    // Encoder push button
const uint8_t ENC_A = 34;       // Encoder signal A
const uint8_t ENC_B = 35;       // Encoder signal B
//const uint8_t BTN_BACK = 25;    // Back button
const uint8_t BTN_CONFIRM = 33; // Confirm button
...
void loop() {
  ...
  if ((encPush != lastEncPush && encPush == LOW) ||
    ...
    encoderValue = 0;
    //tft.drawCentereString(encoderValue);
...
void analogMeter()
{
  ...
  for (int i = -50; i < 51; i += 5) {
    ...
    // Short scale tick length ขีดสั้น
    if (i % 25 != 0) tl = 8;
    ...
    // Now draw the arc of the scale เส้นโค้งในมิเตอร์
  ...
  tft.drawString("V-Meter", 5 + 230 - 40, 119 - 20, 2); // Units at bottom right
  tft.drawCentreString("Volts", 120, 70, 4); // Comment out to avoid font 4
...
void plotNeedle(int value, byte ms_delay)
{
  ...
  char buf[8]; dtostrf(encoderValue, 4, 0, buf);
  tft.drawRightString(buf, 40, 119 - 20, 2);

  if (encoderValue < -10) encoderValue = -10; // Limit value to emulate needle end stops
  if (encoderValue > 110) encoderValue = 110;

  // Move the needle util new value reached
  while (!(encoderValue == old_analog)) {
    if (old_analog < encoderValue) old_analog++;
    else old_analog--;

    if (ms_delay == 0) old_analog = encoderValue; // Update immediately id delay is 0
    ...
    if (abs(old_analog - encoderValue) < 10) ms_delay += ms_delay / 5;
